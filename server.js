...
const localRedisMock = {
  ...
  async srem(key, value) {
    if (this.data.has(key)) {
      this.data.get(key).delete(value);
    }
    return 1;
  },
...
if (redisUrl && redisToken) {
  try {
    ...
    console.warn('[CACHE] Redis connection failed! Falling back to IN-MEMORY CACHE.', err.message);
    redis = localRedisMock;
  }
} else {
  console.warn('[SYSTEM] Redis credentials missing. Falling back to IN-MEMORY CACHE.');
  redis = localRedisMock;
}
...
app.post('/api/auth/signup', async (req, res) => {
  try {
    ...
    let user;
    if (isMongoConnected) {
      user = new User({ username: username.toLowerCase(), password: hashedPassword, avatar });
      await user.save();
    } else {
      user = await localDbMock.createUser(username.toLowerCase(), hashedPassword, avatar);
    }
    ...
    // Broadcast new user registration to all active WebSocket clients
    if (wss) {
      try {
...
// ============================================================
// 4. WEBSOCKET SERVER & REALTIME LOGIC
// ============================================================

const server = http.createServer(app);
const wss = new WebSocket.Server({ server });

// Map of active connections: username -> Set of sockets
...
wss.on('connection', async (ws, req) => {
  ...
  try {
    ...
    console.log('[WS] Connection rejected: Unauthenticated client.', err.message);
  ...
  try {
    ...
    if (cachedRaw && cachedRaw.length > 0) {
      ...
      if (isMongoConnected) {
        ...
        const mockMsgs = await localDbMock.getRecentMessages('lounge', 50);
        history = mockMsgs.map(m => ({
          type: 'chat',
          roomId: 'lounge',
          username: m.sender,
          message: m.content,
      ...
      for (const msg of history) {
        await client.rpush(cacheKey, JSON.stringify(msg));
      }
  ...
  ws.on('message', async (rawData) => {
    try {
      ...
      if (parsedData.type === 'get_history') {
        ...
        if (cachedRaw && cachedRaw.length > 0) {
          ...
          if (isMongoConnected) {
            ...
            const mockMsgs = await localDbMock.getRecentMessages(targetRoomId, 50);
            history = mockMsgs.map(m => ({
              type: 'chat',
              roomId: targetRoomId,
              username: m.sender,
      ...
      // Push to Redis Cache
      const cacheKey = `chat_history:${targetRoomId}`;
      await client.rpush(cacheKey, JSON.stringify(payload));
      await client.ltrim(cacheKey, -50, -1);
      ...
      if (targetRoomId === 'lounge') {
        ...
        const parts = targetRoomId.substring(3).split('_');
        const users = [username, parts.find(p => p !== username)];
        users.forEach(u => {
          const socketSet = userSockets.get(u);
          if (socketSet) {
            socketSet.forEach(s => {
...
// REST push notification route (Admin broadcast)
